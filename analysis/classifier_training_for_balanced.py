#!/usr/bin/env python3
"""Train an mmBERT EU-party classifier on a class-balanced train split.

Train, dev and test are all uniform across parties, so the model is trained with
plain cross-entropy and no imbalance correction. Dev and test are scored through
the identical argmax path, which keeps the two numbers directly comparable.
"""
import json
import os
import sys
...
OUTPUT_DIR = f"{MODEL_SLUG}-balanced"
...
def main():
    ...
    output_tag = f"{MODEL_SLUG}_balanced_ep{best_epoch}_len{MAX_LEN}"
