...
def assign_statements(cache, df, embedder, statements):
    ...
    out = pd.DataFrame(index=df.index, columns=["statement_idx", "statement_sim"], dtype=float)
    for lang, block in df.groupby("language"):
        ...
        best = sims.argmax(1)
        out.loc[block.index, "statement_idx"] = idx_arr[best]
        out.loc[block.index, "statement_sim"] = sims[np.arange(len(best)), best]
    out["statement_idx"] = out["statement_idx"].astype(int)
    return out


def assign_clusters(cache, df, k, seed=0):
    from sklearn.cluster import MiniBatchKMeans

    # language-center first so clusters key on topic, not language
    emb = cache.get(df["id"].tolist()).astype(np.float64)
    transform = FeatureTransform(kind="lang_center").fit(emb, groups=df["language"].to_numpy())
    proj = transform.apply(emb, df["language"].to_numpy())
...
def main():
    ...
    for split in args.splits.split(","):
        ...
        if args.topics == "statements":
            ...
            ensure_embedded(cache, embedder, df["text"].tolist())
            assigned = assign_clusters(cache, df, args.clusters)
